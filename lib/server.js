...
server.init = () => {
    server.httpServer.listen(config.httpPort, () => { console.log(`The HTTP server is running on port: ${config.httpPort}`); });
    server.httpsServer.listen(config.httpsPort, () => { console.log(`The HTTPS server is running on port: ${config.httpsPort}`); });
}
