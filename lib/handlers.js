...
handlers.index = (data, callback) => {
    ...
    if (data.method == 'get') {
        const templateData = {
            'head.title' : 'Monitoramento de Disponibilidade',
            'head.description' : 'Oferecemos monitoramente gratuito e simples de sites HTTP/HTTPS de todo os tipos. Quando o seu site cair, nós enviaremos um SMS para que você fique sabendo.',
            'body.class' : 'index'
        };
            ...
            if (!err && str ) {
                helpers.addUniversalTemplates(str, templateData, (err, str) => {
                    if (!err && str) {
                        ...
                        console.log("Error in addding universal template:", err, "string:", str)
                        callback(500, undefined,'html');
                    }
                });
            } else {
                console.log("Error in getting template:", err, "string:", str)
                callback(500, undefined, 'html');
            }
        });
    } else {
        console.log("Homepage only accepts GET requests.")
